...
const S = { project: null, digest: "", frame: 0, selected: null, dirty: false, playing: false, timer: null, previewToken: 0 };
...
function toast(msg, error = false) {
  ...
  clearTimeout(el._timer); el._timer = setTimeout(() => { el.className = ""; }, 2600);
}
...
function uid(prefix) {
  const ids = new Set([...(S.project.layers || []).map(x => x.id), ...(S.project.captions || []).map(x => x.id), ...(S.project.audio || []).map(x => x.id)]);
...
function objectRow(type, row, meta) {
  ...
  const name = document.createElement("span"); name.textContent = row.id; const m = document.createElement("span"); m.className = "meta"; m.textContent = meta; div.append(name, m); div.onclick = () => setSelection(type, row.id); return div;
}
...
async function finalRender() { if(!await normalizeProject(false))return; const btn=$("#renderBtn");btn.disabled=true;btn.textContent="Rendering…";try{const name=`${S.project.id}-${Date.now()}`;const r=await api("/api/render",{project:S.project,name,profile:"h264",mode:$("#realizationMode").value});$("#evidence").textContent=JSON.stringify(r,null,2);toast(`Render complete: ${r.output}`);}catch(e){toast(e.message,true);}finally{btn.disabled=false;btn.textContent="Render Final";} }
...
function bind() {
  $$('[data-add-layer]').forEach(b=>b.onclick=()=>addLayer(b.dataset.addLayer)); $("#addCaptionBtn").onclick=addCaption; $("#addToneBtn").onclick=addTone; $("#addSpeechBtn").onclick=addSpeech; $("#duplicateBtn").onclick=duplicateSelected; $("#deleteBtn").onclick=deleteSelected;
  ...
  document.addEventListener("keydown",e=>{if((e.ctrlKey||e.metaKey)&&e.key.toLowerCase()==="s"){e.preventDefault();save();}if(e.code==="Space"&&!["INPUT","TEXTAREA"].includes(document.activeElement.tagName)){e.preventDefault();togglePlay();}});
}

async function boot() {
  bind(); try { const st=await api("/api/state");S.project=st.project;S.digest=st.project_digest;S.frame=0;markDirty(false);renderAll();$("#jsonEditor").value=JSON.stringify(S.project,null,2);$("#evidence").textContent=JSON.stringify({machine:st.machine,project_path:st.project_path,truth_boundary:st.truth_boundary},null,2);preview(); } catch(e){toast(e.message,true);$("#evidence").textContent=e.stack||e.message;}
}
boot();
