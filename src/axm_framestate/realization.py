...
POLICY_SCHEMA = 'axm.framestate.realization-policy/v0.1'
CONTRACT_SCHEMA = 'axm.framestate.render-contract/v0.1'
...
def _memory_mb() -> int:
    # Bounded host observation only. It is never canonical project truth.
    try:
...
def probe_machine() -> dict[str, Any]:
    """Return only bounded execution facts needed by v0.9 planning.

    Deliberately omitted: usernames, paths, serials, MAC/network data, device IDs,
    browser fingerprints and unimplemented GPU claims.
    """
    raw = {
        ...
        # CPU software is the only FrameState-owned render backend in v0.9.
...
def normalize_machine_capabilities(raw: Any) -> dict[str, Any]:
    ...
    if 'cpu-software' not in backends:
        raise RealizationError('v0.9 requires cpu-software as an executable fallback backend')
    result = {
        'schema': MACHINE_SCHEMA,
        'id': str(raw.get('id', 'declared-machine'))[:200],
        'logical_cores': _int(raw.get('logical_cores', 1), 'logical_cores', 1, 4096),
        # 0 means unknown. Unknown is treated conservatively by the planner.
...
def normalize_realization_policy(raw: Any | None) -> dict[str, Any]:
    ...
    if raw.get('schema', POLICY_SCHEMA) != POLICY_SCHEMA:
        raise RealizationError(f'schema must be {POLICY_SCHEMA}')
    ...
    result = {
        ...
        # Effects can carry important expression, so this is opt-in rather than default degradation.
...
def _tier(machine: dict[str, Any], mode: str) -> str:
    ...
    # Unknown memory must not silently promote a host into a richer tier.
...
def plan_realization(project: dict[str, Any], machine: dict[str, Any], policy: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    contract = {
        ...
        'truth_boundary': (
            'v0.9 adapts only FrameState-owned expression work: particle density, deterministic 3D shadow work, '
            'optional effect-pass budget when user policy explicitly permits it, and compatibility export profile. '
            'It does not claim GPU backend selection, semantic scene simplification, or adaptive canonical truth.'
        ),
    }
