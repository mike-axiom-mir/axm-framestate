...
from .native_image import PNG_SIGNATURE, decode_png
...
def _confined_directory(output_root:Path,*parts:str)->Path:
    ...
    for part in parts:
        ...
        try:
            ...
            raise MediaError(f'media output escapes render root: {current}') from e
...
def _confined_file(output_root:Path,path:Path)->Path:
    ...
    if path.is_symlink():
        raise MediaError(f'media output file must not be a symlink: {path}')
    if path.exists() and not path.is_file():
        raise MediaError(f'media output file must be regular: {path}')
    try:
        path.resolve().relative_to(root)
    except ValueError as e:
        raise MediaError(f'media output escapes render root: {path}') from e
    return path
...
def _pillow_image():
    try:
        ...
        raise MediaError('Pillow is required only for non-native imported image/font compatibility boundaries') from exc
...
def _ppm_from_rgb(width:int,height:int,rgb:bytes)->bytes:
    if width<1 or height<1 or len(rgb)!=width*height*3: raise MediaError('invalid RGB image')
    return f'P6\n{width} {height}\n255\n'.encode('ascii')+rgb

def _parse_ppm(raw:bytes)->tuple[int,int,bytes]:
    if not raw.startswith(b'P6'): raise MediaError('not native PPM')
    try: head,body=raw.split(b'\n255\n',1); parts=head.split(); w,h=int(parts[1]),int(parts[2])
    except (ValueError,IndexError) as exc: raise MediaError('invalid PPM') from exc
    if w<1 or h<1 or len(body)!=w*h*3: raise MediaError('invalid PPM body')
    return w,h,body

def decode_image_bytes(raw:bytes)->tuple[int,int,bytes,dict[str,Any]]:
    if raw.startswith(b'P6'):
        w,h,rgb=_parse_ppm(raw); return w,h,rgb,{'boundary':'native-ppm'}
    if raw.startswith(PNG_SIGNATURE):
        try: return decode_png(raw)
        except ValueError as exc: raise MediaError(str(exc)) from exc
    Image=_pillow_image()
    try:
        with Image.open(io.BytesIO(raw)) as im:
            rgb=im.convert('RGB'); return rgb.width,rgb.height,rgb.tobytes(),{'boundary':pillow_version(),'format':im.format}
    except Exception as exc:
        raise MediaError('image could not be decoded by native PNG/PPM or optional Pillow boundary') from exc

def read_ppm(path:Path)->tuple[int,int,bytes]:
    raw=Path(path).read_bytes(); w,h,rgb,_=decode_image_bytes(raw); return w,h,rgb
...
def conform_media(project:dict[str,Any], output_dir:Path, machine_root:Path)->dict[str,Any]:
    ...
    for m in project.get('media',[]):
        ...
        if m['kind']=='image':
            w,h,rgb,boundary=decode_image_bytes(src.read_bytes()); data=_ppm_from_rgb(w,h,rgb)
            d=_confined_directory(output_dir,'conformed-media',m['id']); p=_confined_file(output_dir,d/'frame-000000.ppm'); p.write_bytes(data)
            evidence.update({'width':w,'height':h,'frame_count':1,'conformed_digest':file_digest(p),**boundary})
        elif m['kind']=='video':
            exe=shutil.which('ffmpeg')
        ...
        elif m['kind']=='mesh': evidence.update({'bytes':src.stat().st_size,'boundary':'native-obj-parser'})
    ...
    result={'schema':'axm.framestate.media-manifest/v0.5','rows':rows}; result['manifest_digest']=digest(result)
...
class MediaCache:
    def __init__(self,project:dict[str,Any],output_dir:Path,machine_root:Path):
        ...
        self.manifest=conform_media(project,output_dir,machine_root); self._ppm={}; self._mesh={}; self._child={}
    ...
    def frame_count(self,mid:str)->int:
        ...
        if item['kind']=='framestate': return self.child(mid)['project']['duration_frames']
        return 0
    def image_frame(self,mid:str,index:int)->tuple[int,int,bytes]:
        item=self.item(mid); key=(mid,index)
    ...
    def mesh(self,mid:str):
        ...
        item=self.item(mid); from .three_d import parse_obj
    ...
    def child(self,mid:str)->dict[str,Any]:
        ...
        path=resolve_source(self.root,item['path']); project=load_project(path); child_out=_confined_directory(self.output_dir,'child',mid)
        from .receipts import render_with_receipt
        receipt=render_with_receipt(project,child_out,self.root,assemble=False); result={'project':project,'output':child_out,'receipt':receipt}; self._child[mid]=result; return result